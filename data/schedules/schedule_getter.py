...
divisions = {'heroic': 'H', 'nexus': 'N', 'a': 'A',
             'b-northeast': 'BNE', 'b-southeast': 'BSE', 'b-west': 'BW', 'c-east': 'CE',
             'c-west': 'CW', 'd-northeast': 'DNE', 'd-southeast': 'DSE',
             'd-west': 'DW', 'e-east': 'EE', 'e-west': 'EW'}

workbook = Workbook()

url = "https://nexusgamingseries.org/api/schedule/fetch/division/matches/"

for div in divisions.keys():
    print("=============", div, "=============")
    r = requests.post(url, data={"season": 12, "division": div})
    ...
    for match in matches:
        ...
        home_ticker = ""
        away_ticker = ""
        # if "challonge_match_ref" in match:
        #    continue
        try:
